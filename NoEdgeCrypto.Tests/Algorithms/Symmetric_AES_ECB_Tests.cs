...
{
    [TestClass]
    public class Symmetric_AES_ECB_Tests
        : BaseTest
    {
        [TestInitialize]
        ...
        [DataRow(Symmetric_AES.KeySize.KeySize128Bit)]
        [DataRow(Symmetric_AES.KeySize.KeySize256Bit)]
        public void TestSameInputProducesSameOutput(Symmetric_AES.KeySize keySize)
        {
            var key = Symmetric_AES.GenerateRandomKey(keySize);
            for (int counter = 0; counter < 10; ++counter)
            {
                ...
                Assert.AreEqual(1, encryptedAsBase64.Count, "Should be only 1 distinct value");
            }
        }
                ...
                using (var outputStream = numBytes.HasValue ? new MemoryStream() : null)
                {
                    using (var algorithm = new Symmetric_AES_ECB())
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                Assert.AreEqual(typeof(ArgumentNullException), ex.GetType(), "Exception type");
                            }
                        }
                    }
                }
